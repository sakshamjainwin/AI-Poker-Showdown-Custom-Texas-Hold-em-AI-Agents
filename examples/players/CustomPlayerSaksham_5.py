...
class CustomPokerPlayer(BasePokerPlayer):
    ...
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        if play_random in (69, 76, 66, 16, 32, 18, 99, 7, 62, 19, 44, 11, 45):
            ...
            return "raise", min(valid_actions[2]["amount"]["max"]*0.75, random.randint(valid_actions[2]["amount"]["min"], valid_actions[2]["amount"]["min"]*4))
        ...
        if hole_card[0][1] == hole_card[1][1] and len(commc) == 0: 
            if hole_card[0][1] in ["T", "K", "Q", "J","A"]:
                if valid_actions[1]['amount'] < 150:
                    return valid_actions[2]['action'], min(valid_actions[2]["amount"]["max"]*0.75, valid_actions[2]['amount']['min']*3)
            else:
                if valid_actions[1]['amount'] < 250:
                    return valid_actions[2]['action'], min(valid_actions[2]["amount"]["max"]*0.75, valid_actions[2]['amount']['min']*2)
        ...
        if len(commc) > 0:
            ...
            for i in commc:
                if (hole_card[0][0] == hole_card[1][0]) and i[0] == hole_card[0][0]:
                    ssc += 1
                    if ssc == 5:
                        return "raise", min(valid_actions[2]["amount"]["max"]*0.75, valid_actions[2]["amount"]["min"]*6)

                    if (ssc + rounds_left >= 5):
                        if valid_actions[1]['amount'] < 150:
                            return "raise", min(valid_actions[2]["amount"]["max"]*0.75, valid_actions[2]["amount"]["min"]*ssc)
        ...
        # If the current element is not exactly one more than the previous one, return False
                ...
                if straight_pos == 5:
                    return "raise", valid_actions[2]["amount"]["max"]*0.75

                if (straight_pos + rounds_left >= 5):
                    if valid_actions[1]['amount'] < 250:
                        return "raise", min(valid_actions[2]["amount"]["max"]*0.75, valid_actions[2]["amount"]["min"]*straight_pos)      
